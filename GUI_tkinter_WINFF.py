...
# Copied / adapted from the original repository: https://github.com/mauriciomenon/FFmpegTJSP-WINFF
# This file implements the Tkinter-based GUI for FFmpeg conversion (full feature set).
...
# carregar config inicial se existir
...
def save_config():
    ...
    config['DEFAULT'] = {
        ...
        'use_same_directory': str(use_same_directory_var.get()),
        'overwrite_existing': str(overwrite_var.get())
    }
...
# Seleção de arquivo / diretório / executável
...
# Exibir informações do vídeo via ffprobe
...
# Atualizar exibição do comando FFmpeg

def update_command_display(event=None):
    input_file = input_entry.get()
    output_format = format_var.get()
    video_bitrate = video_bitrate_entry.get()
    audio_bitrate = audio_bitrate_entry.get()
    ...
    output_file = os.path.join(output_dir, os.path.splitext(os.path.basename(input_file))[0] + '.' + output_format) if input_file else ''
...
# Buttons: default, load, save, convert
